...
router.get('/profile', (req, res, next) => {
    try {
        ...
        const profile = mistakeTracker_1.mistakeTracker.getProfile(userId);
        const topErrors = mistakeTracker_1.mistakeTracker.getTopRecurringErrors(userId, 5);
...
router.get('/workout', async (req, res, next) => {
    try {
        const userId = req.user.userId;
        const maxMinutes = parseInt(req.query.maxMinutes) || 12;
        const workout = await mistakeTracker_1.mistakeTracker.generateRemediationWorkout(userId, maxMinutes);
        res.json({
            success: true,
            data: {
                workout,
...
router.post('/workout/:id/complete', (req, res, next) => {
    try {
        const userId = req.user.userId;
        const { id } = req.params;
        ...
        if (results?.correctErrorIds) {
            for (const errorId of results.correctErrorIds) {
                mistakeTracker_1.mistakeTracker.recordCorrectUsage(userId, errorId);
            }
        }
        res.json({
            success: true,
...
router.post('/weekly-update', (req, res, next) => {
    try {
        const userId = req.user.userId;
        mistakeTracker_1.mistakeTracker.weeklyUpdate(userId);
        res.json({
            success: true,
