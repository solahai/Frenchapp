...
router.post('/grammar/explain', rateLimiter_1.aiRateLimiter, async (req, res, next) => {
    try {
        ...
        const explanation = await openai_1.openAIService.explainGrammar(topic, level || 'A1', question);
        res.json({
            success: true,
            data: {
...
router.get('/culture/nugget', (req, res, next) => {
    try {
