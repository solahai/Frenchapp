...
exports.LessonBuilder = exports.lessonBuilder = void 0;
const database_1 = require("./database");
const openai_1 = require("./openai");
const srsEngine_1 = require("./srsEngine");
...
class LessonBuilder {
     ...
     */
    async generateDailyLesson(userId, date, level, durationMinutes = 20, focusAreas, preferredTopics) {
    ...
    `).get(userId, date);
        ...
        const content = await openai_1.openAIService.generateLessonContent(level, theme, learnedVocab.map(v => v.french), this.selectGrammarFocus(userId, level));
        // Build lesson structure
        const lessonId = `lesson_${(0, uuid_1.v4)()}`;
        const sections = this.buildSections(durationMinutes, recallItems, content, level);
        const lesson = {
            id: lessonId,
            userId,
            date,
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).run(lessonId, userId, date, level, 'scheduled', JSON.stringify({
            sections,
            vocabulary: content.vocabulary,
            grammarRule: content.grammarRule,
            pronunciationTarget: lesson.pronunciationTarget,
            culturalNugget: content.culturalNugget,
        }), JSON.stringify(lesson.metrics), new Date().toISOString());
        // Create SRS cards for new vocabulary
        for (const vocab of content.vocabulary) {
            srsEngine_1.srsEngine.createVocabularyCards(userId, `vocab_${vocab.french}`, {
                french: vocab.french,
     ...
     */
    buildSections(durationMinutes, recallItems, content, level) {
        const sections = [];
        // 1. Warm Start (60-90s) - Recall Sprint
        sections.push({
            id: `section_${(0, uuid_1.v4)()}`,
            type: 'warm_start',
            title: 'Recall Sprint',
            durationMinutes: 1.5,
     ...
     */
    buildRecallActivities(items) {
        return items.map(item => ({
            id: `activity_${(0, uuid_1.v4)()}`,
            type: 'recall_sprint',
            skill: 'speaking',
            content: {
     ...
     */
    buildVocabularyActivities(vocabulary) {
        const activities = [];
     ...
     */
    buildGrammarActivity(grammarRule) {
        return {
            id: `activity_${(0, uuid_1.v4)()}`,
     ...
     */
    buildPronunciationActivity(level) {
        const targets = {
        ...
        const target = targets[level] || targets['A1'];
        return {
            id: `activity_${(0, uuid_1.v4)()}`,
     ...
     */
    buildInterleavedPractice(content, level) {
        const activities = [];
        ...
        activities.push({
            ...
            skill: 'writing',
            content: {
                sentences: vocab.slice(0, 3).map((v) => ({
                    english: v.english,
                    french: v.french,
                })),
            },
        ...
        activities.push({
            ...
            skill: 'listening',
            content: {
                sentences: vocab.slice(0, 4).map((v) => v.french),
                task: 'identify',
            },
        ...
        activities.push({
            ...
            content: {
                ...
                sentences: grammar.examples?.map((e) => ({
        ...
        activities.push({
            ...
            skill: 'writing',
            content: {
                words: vocab.slice(0, 3).map((v) => v.french),
     ...
     */
    buildCafeActivity(content, level) {
        const scenarios = [
        ...
        return {
            ...
            content: {
                ...
                targetVocabulary: content.vocabulary.slice(0, 3).map((v) => v.french),
                level,
                strictMode: true,
            },
    ...
    getRecallItems(userId) {
        ...
        ORDER BY RANDOM()
        LIMIT 1
      `).get(this.getCurrentUserId(), startDate.toISOString(), targetDate.toISOString());
            if (item) {
                items.push(item);
            }
        }
    ...
    selectGrammarFocus(userId, level) {
    ...
    selectPronunciationTarget(level) {
        const targets = {
        ...
        return targets[level] || targets['A1'];
    }
    parseLessonFromDb(row) {
...
exports.LessonBuilder = LessonBuilder;
exports.lessonBuilder = new LessonBuilder();
//# sourceMappingURL=lessonBuilder.js.map