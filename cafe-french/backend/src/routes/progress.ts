...
router.get('/weekly-report', (req: Request, res: Response, next: NextFunction) => {
  try {
    const userId = req.user!.userId;
    const weekNumber = parseInt(req.query.week as string) || getCurrentWeekNumber();
    ...
    `).all(userId, weekStart.toISOString().split('T')[0], weekEnd.toISOString().split('T')[0]) as any[];
...
router.post('/challenge/start', (req: Request, res: Response, next: NextFunction) => {
  try {
    const userId = req.user!.userId;
    const { startLevel, targetLevel, startDate } = req.body;

    if (!startLevel || !targetLevel) {
      throw new ValidationError('Start and target levels required');
    }
    ...
    `).run(
      ...
      JSON.stringify(weeklyMilestones),
...
router.get('/challenge', (req: Request, res: Response, next: NextFunction): void => {
  try {
    const userId = req.user!.userId;
    ...
    res.json({
      ...
      data: {
        challenge: {
          ...
          currentDay,
          completedDays: JSON.parse(challenge.daily_plan).filter((d: any) => d.completed).length,
...
router.post('/challenge/:id/day/:day/complete', (req: Request, res: Response, next: NextFunction) => {
  try {
    const userId = req.user!.userId;
    ...
    progressMetrics.lessonsCompleted = dailyPlan.filter((d: any) => d.completed).length;
