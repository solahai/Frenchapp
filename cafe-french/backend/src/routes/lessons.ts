...
import { lessonBuilder } from '../services/lessonBuilder';
...
router.get('/daily', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const lesson = await lessonBuilder.generateDailyLesson(
      userId,
      date,
      level,
...
router.post('/:id/activity/:activityId/complete', (req: Request, res: Response, next: NextFunction) => {
  try {
    const userId = req.user!.userId;
