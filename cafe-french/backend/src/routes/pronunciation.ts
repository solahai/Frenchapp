...
const SHADOWING_PHRASES: Record<string, Array<{ text: string; ipa: string; translation: string }>> = {
  'A1': [
...
router.post('/assess', speechRateLimiter, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const userId = req.user!.userId;
    const { targetText, audioData, level } = req.body;

    if (!targetText || !audioData) {
      throw new ValidationError('Target text and audio data required');
    }

    // Transcribe audio using Whisper
    const audioBuffer = Buffer.from(audioData, 'base64');
    const transcription = await openAIService.transcribeAudio(audioBuffer, 'fr');
    ...
    for (const issue of analysis.phonemeIssues) {
      mistakeTracker.recordPronunciationError(
        userId,
        issue.phoneme,
        issue.phoneme, // IPA would need lookup
        targetText,
        analysis.score
      );
    }

    const assessmentId = `pa_${uuidv4()}`;
...
router.get('/shadowing', (req: Request, res: Response, next: NextFunction) => {
  try {
    const userId = req.user!.userId;
    ...
    const sessionId = `shadow_${uuidv4()}`;
...
router.post('/shadowing/:phraseId/attempt', speechRateLimiter, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const userId = req.user!.userId;
    ...
    if (!audioData || !targetText) {
      throw new ValidationError('Audio data and target text required');
    }
...
router.get('/patterns/:level', (req: Request, res: Response) => {
